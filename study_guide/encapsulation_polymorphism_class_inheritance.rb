...
The three typcial ways that Ruby implements polymorphism is through class
inheritance,  Duck-typing and mixing in Modules to a class. When a class
inherits a method from a super class, the sub class can override the parent
class method by defining an instance method with the same name as the inherited
method. This is polymorphism through inheritance. Polymorphism through
Duck-typing is when different, unrelated objects can respond the same method
name. However, with duck-typing, emphasis should be placed on the sameness of
the behavior of the unrelated objects. So, if something can "quack", treat it
like a duck. Duck-typing is an informal classification of a group of unrelated
objects pulled together by overlapping behaviors, thus duck-typing allows for
polymorphism without direct inheritance. Modules also allow for polymorphic
structure to be implemented in Ruby. Multiple classes can mixin the same module
giving the differing object type the same behavior. Polymorphism should be
understood and implemented to create clearity of intention and use of code.
Merely having a method that can be called by several different object types
does not make for polymorphism even when polymorphic technique is used.
